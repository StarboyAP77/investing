...
export function renderCorrelationMatrix(data, containerId) {
    ...
    export function updateYearlyPerformanceTable(filteredData, dates, targetAlloc, rebNAV, noRebNAV, benchmarkNAV) {
    ...
    export function updateYoYPerformanceChart(performanceData, sortedYears) {
        ...
        const series = Object.keys(seriesData).map(assetName => { let displayName = assetName; if (assetName === 'REBALANCED') displayName = 'Rebalanced'; else if (assetName === 'NON_REBALANCED') displayName = 'Non-Rebalanced'; return { name: displayName, type: 'line', smooth: 0.3, symbol: 'circle', symbolSize: 8, data: seriesData[assetName].map(d => ({ value: d.rank, originalReturn: d.return })), lineStyle: { width: 3, color: ASSET_CHART_COLORS[assetName] || 'black' }, itemStyle: { color: ASSET_CHART_COLORS[assetName] || 'black' } }; });
